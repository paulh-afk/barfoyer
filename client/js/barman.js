...
const fetchApi = (method) => {
  return fetch('http://' + ipServer + '/foyerbdd/' + method);
};

const fetchApiToJson = (method) => {
  const produits = fetch('http://' + ipServer + '/foyerbdd/' + method)
    .then((res) => res.json())
    .then((json) => json)
    .catch(() => null);
  return produits;
...
const commandesTemplate = (commandes) => {
  const commandeTemplate = commandes[0].map((commande, index) => {
    ...
    commandes[2][index].map((detailsCommande) => {
      ...
      produitsElement.innerHTML += `<div class="cmd-list"><h4>${produit.denomination} x${
